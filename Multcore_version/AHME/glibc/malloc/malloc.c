...
#define NVM_MMAP(addr, size, prot, flags) \
 __mmap((addr), (size), (prot), (flags)|MAP_ANONYMOUS|MAP_PRIVATE|MAP_NVM, -1, 0)



static void *sysnvmmalloc (INTERNAL_SIZE_T, mstate);

static void *
sysnvmmalloc (INTERNAL_SIZE_T nb, mstate av)
{
  mchunkptr old_top;              /* incoming value of av->top */
  INTERNAL_SIZE_T old_size;       /* its size */
  char *old_end;                  /* its end address */
  ...
  if ((unsigned long) (nb) >= (unsigned long) (mp_.mmap_threshold) &&
    ...
    try_mmap:
      ...
      if ((unsigned long) (size) > (unsigned long) (nb))
        {
          mm = (char *) (NVM_MMAP (0, size, PROT_READ | PROT_WRITE, 0));
    ...
    { /* Request enough space for nb + pad + overhead */
      ...
      if (brk != (char *) (MORECORE_FAILURE))
      ...
      else
        {
          ...
          if ((unsigned long) (size) > (unsigned long) (nb))
            {
              char *mbrk = (char *) (NVM_MMAP (0, size, PROT_READ | PROT_WRITE, 0));
...
static void*  _int_nvm_malloc(mstate, size_t);

static void *
_int_nvm_malloc (mstate av, size_t bytes)
{
  INTERNAL_SIZE_T nb;               /* normalized request size */
  unsigned int idx;                 /* associated bin index */
  mbinptr bin;                      /* associated bin */
...
#define MAX_ITERS       10000
    ...
    use_top:
      /*
         ...
         exhausted by current request, it is replenished. (The main
         reason for ensuring it exists is that we may need MINSIZE space
         to put in fenceposts in sysnvmmalloc.)
       */
...
void*  __libc_nvm_malloc(size_t);
libc_hidden_proto (__libc_nvm_malloc)

void *
__libc_nvm_malloc (size_t bytes)
{
  mstate ar_ptr;
  void *victim;
  ...
  victim = _int_nvm_malloc (ar_ptr, bytes);
  if (!victim)
    {
      LIBC_PROBE (memory_malloc_retry, 1, bytes);
      ar_ptr = arena_get_retry (ar_ptr, bytes);
...
libc_hidden_def (__libc_nvm_malloc)

strong_alias (__libc_nvm_malloc, __nvm_malloc) strong_alias (__libc_nvm_malloc, nvm_malloc)
